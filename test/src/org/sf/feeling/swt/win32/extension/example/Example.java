...
public class Example
{
	...
	private void initial()
	{
		...
		shell
				.setImages(new Image[] { ImageCache.getImage("/eclipse.png"),
		...
		shell.setText("Eclipse SWT Extension Example");
		menubar = new MenuBar(shell, SWT.NONE);

		menubar.getControl().setLayoutData(new GridData(GridData.FILL_HORIZONTAL));
		...
		toolbar.getControl().setLayoutData(new GridData(GridData.FILL_HORIZONTAL));
		...
		shell.addDisposeListener(new DisposeListener()
		{
			public void widgetDisposed(DisposeEvent arg0)
			{
				DIRTY = true;
				manager.dispose();
				if (Hook.MOUSE.isInstalled()) Hook.MOUSE.uninstall();
				if (Hook.KEYBOARD.isInstalled()) Hook.KEYBOARD.uninstall();
				if (JournalRecordHook.isInstalled()) JournalRecordHook.unInstallHook();
				if (JournalPlaybackHook.isInstalled()) JournalPlaybackHook.unInstallHook();
		...
		aboutItem.setText("About SWT Extension");
	...
	private void createHelpMenu()
	{
		...
		SelectionListener homeListener = new SelectionAdapter()
		{
			...
			{
				Program.launch("http://www.swtui.cn");
			}
		...
		CMenuItem about = new CMenuItem("&About SWT Extension", SWT.NONE);
		menu.addItem(about);
	...
	private void createGlossyItem(CMenu menu)
	{
		glossyStyle = new CMenuItem("&Glossy Style", SWT.CHECK)
		{
			...
			{
				return wrapper != null && wrapper.isThemeInstalled()
		...
		helloWorld.addSelectionListener(new SelectionAdapter()
		{
			public void widgetSelected(SelectionEvent e)
			{
				if (example == null)
				{
					...
					example.getShell().addDisposeListener(new DisposeListener()
					{
	...
	private void createTaskbarItem(CMenu menu)
	{
		...
		final CMenuItem alwaysOnTop = new CMenuItem("&Always &On Top", SWT.CHECK)
		{
			public boolean getSelection()
			{
		...
		SelectionListener listener = new SelectionAdapter()
		{
			public void widgetSelected(SelectionEvent e)
			{
				if (e.data instanceof CMenuItem)
				{
					...
					final boolean isAlwaysOnTop = item == alwaysOnTop ? !alwaysOnTop
							.getSelection() : alwaysOnTop.getSelection();
					final boolean isAutoHidden = item == autoHidden ? !autoHidden
							.getSelection() : autoHidden.getSelection();
					Display.getDefault().asyncExec(new Runnable()
					{
						public void run()
						{
							...
							{
								ApplicationBar.setAppBarState(shell.handle, Win32.STATE_NONE);
							}
						}
					});
	...
	private void createOnTopItem(CMenu menu)
	{
		...
		SelectionListener listener = new SelectionAdapter()
		{
			public void widgetSelected(SelectionEvent e)
			{
				if (e.data == onTopItem) Windows.setWindowAlwaysOnTop(shell.handle, true);
				else if (e.data == notOnTopItem) Windows.setWindowAlwaysOnTop(shell.handle,
						false);
			}
	...
	{
		if (Win32.getWin32Version() < Win32.VERSION(5, 0)) return;
		...
		final CMenuItem transparentItem = new CMenuItem("&Non Transparent", SWT.NONE);
		...
		SelectionListener listener = new SelectionAdapter()
		{
			public void widgetSelected(SelectionEvent e)
			{
				...
				{
					CMenuItem item = (CMenuItem) e.data;
					if (item == transparentItem) Windows.setWindowTransparent(shell.handle,
							false);
					else if (item == level90) Windows.setWindowTransparent(shell.handle,
							(byte) (255 * 0.9));
					else if (item == level75) Windows.setWindowTransparent(shell.handle,
							(byte) (255 * 0.75));
					else if (item == level50) Windows.setWindowTransparent(shell.handle,
							(byte) (255 * 0.50));
				}
			}
		};
	...
	private void createPageMenu(final BasePage page, ICategoryProvider provider)
	{
		...
		menu.addMenuListener(new MenuAdapter()
		{
			...
			public void menuShown(Event e)
			{
				if (e.data != null && e.data instanceof MenuControl)
				{
					...
					{
						menu.getItem(i).setSelection(page.getSelectionIndex() == i);
					}
		...
		{
			CMenuItem cItem = new CMenuItem(pages[i].getDisplayLabel(), SWT.CHECK);
