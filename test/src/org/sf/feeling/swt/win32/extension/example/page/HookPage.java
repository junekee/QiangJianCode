...
public class HookPage extends SimpleTabPage {
	...
	public void deActivate() {
		Windows.setWindowAlwaysOnTop(container.getShell().handle,
				false);
	...
	private void createMouseHookArea() {
		Section mouseHookSection = WidgetUtil.getToolkit().createSection(
		...
		Composite mouseHookArea = WidgetUtil.getToolkit().createComposite(
				container.getBody());
		...
		final Label label = WidgetUtil.getToolkit().createLabel(mouseHookArea,
				"Hook is not installed", SWT.CENTER);
		GridData data = new GridData(GridData.FILL_HORIZONTAL);
		data.horizontalSpan = 2;
		data.widthHint = 460;
		label.setLayoutData(data);

		final Button installButton = WidgetUtil.getToolkit().createButton(
				mouseHookArea, SWT.PUSH, true);
		...
		final Button unInstallButton = WidgetUtil.getToolkit().createButton(
				mouseHookArea, SWT.PUSH, true);
	...
	private void createTitle() {
		WidgetUtil
				.createFormText(
						container.getBody(),
						"This page demonstrates SWT Extension ability to install Windows Hooks, using the Hooks API.");
	}
