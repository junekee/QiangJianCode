...
public class RegistryPage extends SimpleTabPage {
	...
	private void createRegistryArea() {
		...
		treeComposite.setBackground(WidgetUtil.getToolkit().getColors()
				.getForeground());
		GridLayout layout = new GridLayout();
		...
		registryViewer.getTree().setLayoutData(
		...
		registryTable.getTable().setLayoutData(
				new GridData(GridData.FILL_BOTH));
	...
	private void createTitle() {
		Label label = WidgetUtil
				...
				.createLabel(
						container,
						"This page demonstrates a simple registry viewer created using RegistryKey class methods.",
						SWT.WRAP);
