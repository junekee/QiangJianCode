...
public class TabbedPropertyList extends Composite
{
	...
	public class ListElement extends Canvas
	{
			...
			addMouseListener(new MouseAdapter()
			{
				...
				public void mouseUp(MouseEvent e)
				{
					if (!selected)
					{
						...
						// while ( !( tabbedPropertyComposite instanceof
						// BaseAttributePage ) )
						// {
		...
		private void paint(PaintEvent e)
		{
			...
			{
				e.gc.setForeground(border);
				if (getSelectionIndex() != NONE && getSelectionIndex() + 1 == index)
				{
					e.gc.drawLine(4, 0, bounds.width - 1, 0);
				}
				else
	...
	public void setElements(Tab[] children)
	{
		...
		else
		{
			...
			for (int i = 0; i < children.length; i++)
			{
				...
				{
					String label = children[i].getText();
					if (getTextDimension(label).x > getTextDimension((children[i].getText())).x)
					{
						widestLabelIndex = i;
					}
				}
	...
	public Point computeSize(int wHint, int hHint, boolean changed)
	{
		...
		else
		{
			 ...
			 */
			result.x = width + 32;
			result.x = result.x >= 150 ? result.x : 150;
		}
		return result;
	}
	...
	private void initColours()
	{
		...
		hoverBackground = new Color(Display.getCurrent(), rgb); //$NON-NLS-1$

		defaultForeground = Display.getCurrent().getSystemColor(SWT.COLOR_WIDGET_FOREGROUND);
		activeBackground = Display.getCurrent().getSystemColor(SWT.COLOR_LIST_BACKGROUND);
		border = Display.getCurrent().getSystemColor(SWT.COLOR_WIDGET_NORMAL_SHADOW);
		darkShadow = Display.getCurrent().getSystemColor(SWT.COLOR_WIDGET_DARK_SHADOW);
		textColor = Display.getCurrent().getSystemColor(SWT.COLOR_WIDGET_FOREGROUND);
	}
