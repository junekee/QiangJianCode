...
public class ShellLinkPage extends SimpleTabPage
{
	...
	{
		Section resolveLinkSection = WidgetUtil.getToolkit().createSection(
		...
		final Text fileText = WidgetUtil.getToolkit().createText(fileSelectedArea, "");
		...
		button.addSelectionListener(new SelectionAdapter()
		{
			public void widgetSelected(SelectionEvent e)
			{
				...
				dialog.setFilterExtensions(new String[] { "*.lnk" });
		...
		final Button resolveButton = WidgetUtil.getToolkit().createButton(fileSelectedArea,
				SWT.PUSH, true);
		...
		resolveButton.addSelectionListener(new SelectionAdapter()
		{
			public void widgetSelected(SelectionEvent e)
			{
				String file = ShellLink.getShortCutTarget(fileText.getText().trim());
				String arguments = ShellLink.getShortCutArguments(fileText.getText().trim());
				if (file != null && file.length() > 0)
				{
					messageLabel.setText("Resolve link successful.", false, false);
					targetLabel.setText("Link target: " + file, false, false);
					if (arguments != null && arguments.trim().length() > 0) argumentsLabel
							.setText("Link arguments: " + arguments, false, false);
					else
	...
	{
		Section createLinkSection = WidgetUtil.getToolkit().createSection(container.getBody(),
				Section.EXPANDED);
		TableWrapData td = new TableWrapData(TableWrapData.FILL);
		...
		final Button createButton = WidgetUtil.getToolkit().createButton(fileSelectedArea,
				SWT.PUSH, true);
		...
		createButton.addSelectionListener(new SelectionAdapter()
		{
			public void widgetSelected(SelectionEvent e)
			{
				...
				if (ShellLink.createShortCut(fileText.getText(), ShellFolder.DESKTOP
						.getAbsolutePath(container.handle)
						+ File.separator + fileName + ".lnk", "Created link by SWT Extension") == 0)
				{
					messageLabel.setText("Create link successful.", false, false);
				}
				else
	...
	private void createTitle()
	{
		WidgetUtil
				.createFormText(container.getBody(),
						"This page demonstrates SWT Extension ability to create and resolve link files.");
	}
