...
public class FilePage extends SimpleTabPage implements CDDriveChangeListener
{
	...
	private void createFileSystemInfo()
	{
		...
		Composite fileInfoClient = WidgetUtil.getToolkit().createComposite(fileInfoSection);
		...
		for (int i = 0; i < drives.length; i++)
		{
			...
			CLabel driveLabel = WidgetUtil.getToolkit().createCLabel(fileInfoClient, "");
			driveLabel.setData(drives[i]);
			driveLabel.setImage(ShellIcon.getFileAssociatedIcons(driveLabel.handle, new File(
					drives[i])));
			driveLabel.setText(FileSystem.getVolumeLabel(drives[i]) + " (" + drives[i] + ")");
			...
			if (diskType == FileSystem.DRIVE_TYPE_CDROM)
			{
				...
				final int driveId = CDDrive.getDeviceID(drives[i].substring(0, 2));

				Button button = WidgetUtil.createButton(fileInfoClient, "Eject");
				GridData gd = new GridData();
				gd.widthHint = 60;
				button.setLayoutData(gd);

				button.setData(new Integer(driveId));
				if (CDDrive.isDoorOpened(driveId)) button.setText("Load");
				...
				button.addSelectionListener(new SelectionAdapter()
				{
					...
					{
						if (!CDDrive.isDoorOpened(driveId)) CDDrive.openDoor(driveId);
						else
				...
				if (OSVersionInfo.getInstance().getMajor() >= 6) button.setVisible(false);
			}
			else
				WidgetUtil.createLabel(fileInfoClient);
		}
				...
				Display.getDefault().asyncExec(new Runnable()
				{
					public void run()
					{
						for (int i = 0; i < cdDriveList.size(); i++)
						{
							...
							driveLabel.setImage(ShellIcon.getFileAssociatedIcons(
									driveLabel.handle, new File(drive)));
							driveLabel.setText(FileSystem.getVolumeLabel(drive) + " (" + drive
									+ ")");
