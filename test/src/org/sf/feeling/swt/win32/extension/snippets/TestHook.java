...
public class TestHook {
	public static void main(String args[]) {
		...
		/*
		Hook.JOURNALRECORD.install();
		Hook.JOURNALRECORD.addListener(new HookEventListener() {
			public void acceptHookData(HookData hookData) {
				if (hookData != null) {
					JournalRecordHookData data = (JournalRecordHookData) hookData;
					if (data.getMessage() == Extension.WM_KEYUP) {
						int j = Extension.GetAsyncKeyState(Extension.VK_SHIFT);
						int i = Extension.MapVirtualKey(data.getParamL(), 2);
						char ch = (char) (i & 0xFF);
						if (j != 0)
							System.out.println(ch + " Key Up with Shift Down");
						else
							System.out.println(ch
									+ " Key Up without Shift Down");
					}
					if (data.getMessage() == Extension.WM_KEYDOWN) {
						int j = Extension.GetAsyncKeyState(Extension.VK_SHIFT);
						int i = Extension.MapVirtualKey(data.getParamL(), 2);
						char ch = (char) (i & 0xFF);
						if (j != 0)
							System.out
									.println(ch + " Key Down with Shift Down");
						else
							System.out.println(ch
